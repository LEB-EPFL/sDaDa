...
function cellInfo = rerunGMAFSnofig(cellInfo)
% function to rerun getMeshAlgForSIM for a dataset, without plotting
% figures, for fast results for debugging
param=initParam;
...
for cellIdx=1:20
    for frIdx=1:num_frames
        skip=false;
        try
            cellList = getMeshAlgForSIM(cellInfo{frIdx, 1}{1, cellIdx}.origCropImg, cellInfo{frIdx, 1}{1, cellIdx}.origCropImg>0, param, cellIdx);
        catch
...
function cellList = getMeshAlgForSIM(img, regions0, p, cellIdx)
...
%                         %plot(xHist,intProf) % or instead run [diamFWHM(ii), ~, ~,~,~, ~, ~]= getFWXM(xHist, intProf', 'searchStep', MINSTEP,'FigZ',debugFigh);
                        ...
                        MINSTEP=0.1 ;% [pixel]INTERPOLATION STEP - dont expect precision better than 0.1*30NM!
